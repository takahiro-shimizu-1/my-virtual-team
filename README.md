...
local で docs / code を実際に生成する場合は `npm run runtime:task -- ai --provider claude|codex|gemini ...` を使います。`codex` subcommand は互換 alias として残しています。
`auto` 選択の考え方は [Execution Policy](docs/execution-policy.md) にまとめています。
...
この repo では GitHub Issue / PR を運用チャネルとして扱います。workflow の中心は route / plan / close と subscription-native agent kickoff で、Issue / PR が開かれると route が解決され、owner / skill / approval_required / required_context が自動コメントされます。
...
local 実行は `Claude Code / Codex / Gemini CLI` を同じ durable task 契約で扱えます。

```bash
npm run runtime:task -- ai --provider claude --prompt "README.md を整備して" --command admin --target-path README.md
npm run runtime:task -- ai --provider codex --prompt "README.md を整備して" --command admin --target-path README.md
npm run runtime:task -- ai --provider gemini --prompt "README.md を整備して" --command admin --target-path README.md
```

GitHub-native 側は custom agent profile 経由で `Copilot default / Claude / Codex` を切り替えられます。`Gemini` は local runner で使います。GitHub-hosted の smoke test は次で実行できます。
...
- [Execution Policy](docs/execution-policy.md): task の性質に応じた Claude / Codex / Copilot / Gemini の選び方
