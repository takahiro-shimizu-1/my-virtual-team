...
npm run runtime:task -- ai --provider codex --task-id task-xxxx
npm run runtime:task -- ai --provider gemini --prompt "README.md を整備して" --command admin --target-path README.md
```

意味:

- `--provider`: `claude` `codex` `gemini` `auto`
- `--prompt`: single-agent fast path を作ってそのまま選択 provider が claim / execute / complete する
- `--task-id`: 既に作成済みの durable task を選択 provider が引き受ける
- `--target-path`: 期待成果物の path。指定した file が生成されなければ fail する
- `--dry-run`: DB を汚さずに route と AI prompt だけ preview する
...
- `github:labels`: GitHub automation に必要な label (`auto`, `copilot`, `claude`, `codex`, `gemini`, `needs-human`) を揃える
- `github:agent-task`: `gh agent-task create` を直接確認する smoke / diagnosis tool
- `ci:verify`: bootstrap + runtime test + representative route/context/AI dry-run smoke + clean worktree を一括確認
...
GitHub bridge が自動で行うのは route / plan / close までで、issue 実装開始は `native-agent-kickoff.yml` の subscription-native kickoff が担当する。local の `runtime:task -- ai ...` は local runner を使いたいときの別経路。
...
- implementation: repo variable `VIRTUAL_TEAM_IMPLEMENTATION_AGENT` or bundled custom agents (`vt-implementation-auto`, `vt-implementation-claude`, `vt-implementation-codex`)
- GitHub-hosted kickoff: issue に `auto` `copilot` `claude` `codex` のいずれかを付ける
...
- `.github/workflows/agent-pr-verify.yml`
  watchdog から explicit dispatch され、open な native agent PR head を default-branch 文脈で検証し、そのまま review handoff と impact report まで流す
...
- `native-agent-watchdog.yml`: open な native agent PR の commit が落ち着いた後に `agent-pr-verify` を自動 dispatch する
