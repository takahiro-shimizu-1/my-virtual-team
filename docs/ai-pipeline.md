...
- `agent-pr-verify.yml`
  open な native agent PR を対象に explicit dispatch され、PR head を default-branch 文脈で checkout して `ci:verify` を実行する
- `native-agent-review.yml`
  `agent-pr-verify` 成功後に既定の native agent mention を投稿する
- `auto-merge.yml`
...
- `gitnexus-impact.yml`
  `agent-pr-verify` 成功後に impacted agents / skills / docs を自動コメントする
...
5. kickoff 後、watchdog が open な native agent PR を検出して `agent-pr-verify` を自動 dispatch する
6. `agent-pr-verify` が成功すると native review handoff と impact comment が走る
