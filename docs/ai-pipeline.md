...
- `agent-pr-verify.yml`
  `Copilot coding agent` 完了後に PR head を default-branch 文脈で checkout し、`ci:verify` を実行する
...
- `native-agent-watchdog.yml`
  managed agent run と review 系 workflow の監視用
- `gitnexus-impact.yml`
...
5. `Copilot coding agent` 完了後に `agent-pr-verify` が PR head を検証する
